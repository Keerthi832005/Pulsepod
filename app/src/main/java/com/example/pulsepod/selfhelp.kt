...
class selfhelp : AppCompatActivity() {
    ...
    private fun fetchPodcastsFromApi() {
        ApiClient.apiService.getselfhelpPodcast().enqueue(object : Callback<List<PodcastModel>> {
            override fun onResponse(
                ...
                if (response.isSuccessful) {
                    val lovePodcasts = response.body()?.filter {
                        it.genre.equals("selfhelp", ignoreCase = true)
                    } ?: emptyList()

                    podcastAdapter.updateData(lovePodcasts)
                } else {
                    Toast.makeText(this@selfhelp, "No podcasts found", Toast.LENGTH_SHORT).show()
                }
            }


            override fun onFailure(call: Call<List<PodcastModel>>, t: Throwable) {
                Toast.makeText(this@selfhelp, "API failed: ${t.message}", Toast.LENGTH_LONG).show()
            }
        })
    }
