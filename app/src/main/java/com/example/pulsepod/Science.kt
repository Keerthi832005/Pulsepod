...
class Science : AppCompatActivity() {
    ...
    private fun fetchPodcastsFromApi() {
        ApiClient.apiService.getSciencePodcast().enqueue(object : Callback<List<PodcastModel>> {
            override fun onResponse(
                call: Call<List<PodcastModel>>,
                response: Response<List<PodcastModel>>
            ) {
                if (response.isSuccessful) {
                    val sciencePodcasts = response.body()?.filter {
                        it.genre.equals("Science", ignoreCase = true)
                    } ?: emptyList()
                    podcastAdapter.updateData(sciencePodcasts)
                } else {
                    Toast.makeText(this@Science, "No podcasts found", Toast.LENGTH_SHORT).show()
                }
            }

            override fun onFailure(call: Call<List<PodcastModel>>, t: Throwable) {
                Toast.makeText(this@Science, "API failed: ${t.message}", Toast.LENGTH_LONG).show()
            }
        })
    }
