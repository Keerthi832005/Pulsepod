...
class Fiction : AppCompatActivity() {
    ...
    private fun fetchPodcastsFromApi() {
        ApiClient.apiService.getFictionPodcast().enqueue(object : Callback<List<PodcastModel>> {
            override fun onResponse(
                ...
                if (response.isSuccessful) {
                    val lovePodcasts = response.body()?.filter {
                        it.genre.equals("Fiction", ignoreCase = true)
                    } ?: emptyList()

                    podcastAdapter.updateData(lovePodcasts)
                } else {
                    Toast.makeText(this@Fiction, "No podcasts found", Toast.LENGTH_SHORT).show()
                }
            }


            override fun onFailure(call: Call<List<PodcastModel>>, t: Throwable) {
                Toast.makeText(this@Fiction, "API failed: ${t.message}", Toast.LENGTH_LONG).show()
            }
        })
    }
