...
class Popculture : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        setContentView(R.layout.activity_popculture) // ✅ use correct layout
    ...
    private fun fetchPodcastsFromApi() {
        ApiClient.apiService.getPopCulturePodcast().enqueue(object : Callback<List<PodcastModel>> {
            override fun onResponse(
                call: Call<List<PodcastModel>>,
                response: Response<List<PodcastModel>>
            ) {
                if (response.isSuccessful) {
                    val popCulturePodcasts = response.body()?.filter {
                        it.genre.equals("Popculture", ignoreCase = true)
                    } ?: emptyList()
                    podcastAdapter.updateData(popCulturePodcasts)
                } else {
                    Toast.makeText(this@Popculture, "No podcasts found", Toast.LENGTH_SHORT).show()
                }
            }

            override fun onFailure(call: Call<List<PodcastModel>>, t: Throwable) {
                Toast.makeText(this@Popculture, "API failed: ${t.message}", Toast.LENGTH_LONG).show()
            }
        })
    }
